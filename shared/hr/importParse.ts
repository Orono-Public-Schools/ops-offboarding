  ...
  for (const row of t.rows) {
    ...
    emp.records.push({
      ...
      tasks: tasksFrom(PROCESS_SPECS.new_hire, {
        ...
        contractSent: doneFromDateCell(t.get(row, 'contract sent')),
        newTeacherFormSent: { done: cellBool(t.get(row, 'new teacher form sent')) },
        ...
        gmailAccount: { done: cellBool(t.get(row, 'school gmail')) },
        synergy: { done: cellBool(t.get(row, 'synergy')) },
        phoneAssigned: { done: cellBool(t.get(row, 'phone')) },
      }),
